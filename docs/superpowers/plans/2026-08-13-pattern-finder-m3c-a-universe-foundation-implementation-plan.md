...
4. The only publication operation is Task 14 `ProfileRegistry.publish(draft, preview, published_at_utc)`. The profile owner itself validates exact Preview/evidence binding; no second service or raw append API exists.
...
No formal manual gate occurs before its commit and independent review PASS. Task 3 has a post-review UI smoke check only; it is not a Manual Functional Acceptance Gate. The first real Manual Functional Acceptance Gate is Task 7.
...
### Task 7: First real per-security functional UI acceptance
...
- [ ] **Step 7: Manual Functional Gate 1** — After review PASS, inspect Common PASS and UNKNOWN/Quarantine securities. Record browser/page, fixture hashes, all visible fields, expected/observed values, reviewer, timestamp and PASS/FAIL.
- [ ] **Step 8: Acceptance record** — Label `FIRST_REAL_MANUAL_FUNCTIONAL_TEST_AFTER_TASK=7`; a FAIL blocks Task 8.
...
- [ ] **Step 1: RED** — Test content-vs-record hash fields, all PASS/FAIL/UNKNOWN rows, provenance sensitivity, deterministic input order, failed/incomplete attempt prohibition for FORMAL, duplicate ID rejection and append-only reads.
- [ ] **Step 2: Confirm RED** — Run `pytest tests/pattern_finder/universe_foundation/test_snapshots.py -q`.
- [ ] **Step 3: GREEN** — Build rows from evaluations and validate exact profile/draft/evidence binding before append/fsync.
...
- [ ] **Step 6: Independent review** — Confirm full-candidate retention, immutable evidence binding, and append-only FORMAL rules. Record PASS before Task 12.
...
- [ ] **Step 6: Independent review** — Confirm projection-only UI and all visible evidence fields. Record PASS.
- [ ] **Step 7: Manual Functional Gate 2** — After review PASS, inspect and record Liquidity, Listing History, Classification, UNKNOWN/Quarantine, raw Industry/Plate, snapshot/evidence, and final membership scopes. A FAIL blocks Task 13.
- [ ] **Step 8: Acceptance evidence record** — Append expected/observed fields, fixture/snapshot hashes, browser/page, timestamp, reviewer and PASS/FAIL; only PASS permits Task 13.
...
**Interfaces:**
- Produces `PreviewDiff`, `PreviewResult`, and `run_preview(*, profile: UniverseProfile | UniverseDraft, gateway_attempt: GatewayAttempt, parent_snapshot: UniverseSnapshot | None, snapshot_store: UniverseSnapshotStore | None=None, universe_snapshot_id: UUID, created_at_utc: datetime) -> PreviewResult`.
- `run_preview()` explicitly loops through `gateway_attempt.evidence`, calls Task 5 `resolve_classification()`, Task 6 `evaluate_security()`, Task 8 `build_funnel()`, and Task 11 `build_snapshot(kind=PREVIEW, ...)`.
- `PreviewResult` freezes `input_profile_version_id: str | None`, `input_draft_id: str | None`, `input_content_sha256: str`, `evidence_attempt_id: str`, `evidence_binding_sha256: str`, `completeness: Completeness`, `snapshot: UniverseSnapshot`, and `diff: PreviewDiff`; Task 14 validates these exact fields.

- [ ] **Step 1: RED** — Test draft/profile hash and evidence binding, COMPLETE/INCOMPLETE, full funnel, added/removed reasons, PREVIEW-only persistence, and unchanged Published/Snapshot/Scan/Review/Detector files.
- [ ] **Step 2: Confirm RED** — Run `pytest tests/pattern_finder/universe_foundation/test_preview.py -q`.
- [ ] **Step 3: GREEN** — Implement using only the four defined earlier interfaces; do not call or define `evaluate_attempt()` or `build_preview_snapshot()`.
...
```powershell
git add src/tv_quant/pattern_finder/universe_foundation/__init__.py src/tv_quant/pattern_finder/universe_foundation/preview.py tests/pattern_finder/universe_foundation/test_preview.py
git commit -m "feat: add non-mutating universe preview"
```

- [ ] **Step 6: Independent review** — Confirm all callees were defined earlier, Preview cannot write FORMAL history, and diff is pure. Record PASS before Task 14.
...
- The registry reloads its stored draft and fail-closes unless draft ID/hash/parent match, preview is COMPLETE, preview Snapshot is PREVIEW, preview profile/draft hash matches exactly, evidence/snapshot hashes validate, change_note is non-empty, filter hash differs from latest family version, and new version allocation is monotonic.
...
- [ ] **Step 1: RED** — Test direct production calls with incomplete/failed Preview, stale or foreign Draft, changed Draft after Preview, mismatched evidence/snapshot/profile/parent hash, blank note, duplicate filter hash, replay and partial-write failure. Assert no alternate append method exists. Test valid CORE:v2 and Custom:v1 while CORE:v1 remains byte-identical.
...
- [ ] **Step 3: GREEN** — Implement exact evidence binding and transaction ordering inside the unique profile owner; do not duplicate evaluator/completeness rules in UI or a second service.
...
- [ ] **Step 6: Independent review** — Confirm `ProfileRegistry.publish()` is the unique production gate and bypassing UI cannot publish incomplete or mismatched evidence. Record PASS before Task 15.
...
**Interfaces:**
- Produces `DraftFormInput`, `PreviewUiState`, `build_draft_form_input(profile: UniverseProfile | UniverseDraft) -> DraftFormInput`, and `build_preview_ui_state(result: PreviewResult) -> PreviewUiState`.
- UI calls `ProfileRegistry` draft methods, `run_preview()`, and `ProfileRegistry.publish()`; it never writes Published storage or decides completeness.
- Produces `render_profile_workflow(*, registry: ProfileRegistry, gateway_attempt: GatewayAttempt, parent_snapshot: UniverseSnapshot | None) -> None`; all dependency types were produced by Tasks 2, 10, 11, and 14. The page composes this function with the unchanged Task 3/7/12 renderers.

- [ ] **Step 1: RED** — Test clone/edit of Exchange, min/max Price, min/max Market Cap, minimum 20D Average Dollar Volume, minimum 20D Average Volume, minimum Listing History, Sector, Industry and ETF/ADR/OTC/Preferred/Warrant/Unit toggles; explicit evidence refresh; preview diff; incomplete publish rejection from service; CORE:v2/Custom:v1 publication; parent immutability; duplicate rejection; rerun no implicit Futu call; and old Snapshot readability.
...
- [ ] **Step 3: GREEN** — Implement forms and display production service errors/reason codes. Publish button invokes the service; visibility/disabled state is presentation only and not a security boundary.
...
- [ ] **Step 6: Independent review** — Confirm UI is not publication/completeness owner and all direct registry publish validation tests pass. Record PASS.
- [ ] **Step 7: Manual Functional Gate 3** — After review PASS, record clone, draft hash/change, Preview evidence/completeness/funnel/diff, incomplete rejection, new version/hash, unchanged parent/history, and visible decision evidence. A FAIL blocks Task 16.
- [ ] **Step 8: Acceptance evidence record** — Append exact parent/draft/Preview/evidence/new-version hashes, expected/observed results, browser/page, timestamp, reviewer and PASS/FAIL; only PASS permits Task 16.
...
| Functional Gate 1 | 7 | First real per-security vertical slice | ticker, evaluation, authoritative metric/evidence, actual, threshold, PASS/FAIL/UNKNOWN, reason, Quarantine, profile/evidence versions, final production membership |
| Functional Gate 2 | 12 | Persisted snapshot evidence | Liquidity, Listing History, Classification, UNKNOWN/Quarantine, raw Industry/Plate, snapshot/evidence, final membership |
| Functional Gate 3 | 15 | Draft → Preview → authorized publication | draft/parent hashes, evidence binding, completeness, diff, incomplete rejection, immutable new version |
| Final | 17 | Integrated M3C-A | all prior gates rerun after final commit/review |

Every formal gate follows commit → independent review PASS → manual UI acceptance → evidence record. `FIRST_UI_SMOKE_AFTER_TASK=3`; `FIRST_REAL_MANUAL_FUNCTIONAL_TEST_AFTER_TASK=7`.
...
- [ ] Dependency order is acyclic: profiles → registry/evidence → classification/evaluator → UI/funnel → adapters/gateway → snapshot → preview → publication → workflow/binding → acceptance.
- [ ] Every consumed interface is produced earlier; no `from_source`, `evaluate_attempt`, or `build_preview_snapshot` reference exists.
- [ ] All constructor/test/example signatures match, including keyword-only Futu gateway construction.
- [ ] `ProfileRegistry.publish()` is the sole production publication/completeness authorization owner; UI remains projection/control wiring and no raw append path exists.
- [ ] Original Task 8 responsibilities are independently reviewable in Tasks 9 and 10 with separate RED/GREEN/review cycles.
- [ ] Formal manual gates occur only after commit and independent review PASS; Task 3 is smoke only and Task 7 is the truthful first functional test.
