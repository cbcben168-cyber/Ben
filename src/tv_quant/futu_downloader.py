...
def futu_to_standardized(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(tickers) != 1 or not tickers.issubset({"SPY", "QQQ"}):
...
def download_futu_daily(code: str, start: date, end: date, quote_context: QuoteContext, *, ret_ok: int = 0, ktype: Any = "K_DAY", autype: Any = "QFQ", sleep: Callable[[float], None] = time.sleep) -> pd.DataFrame:
    ...
    while True:
        ...
        page_key = next_key
    result = futu_to_standardized(pd.concat(pages, ignore_index=True))
...
def update_futu_csv(destination: str | Path, code: str, start: date, end: date, quote_context: QuoteContext, *, ret_ok: int = 0, ktype: Any = "K_DAY", autype: Any = "QFQ", sleep: Callable[[float], None] = time.sleep, before_replace: Callable[[pd.DataFrame], None] | None = None) -> FutuCsvUpdate:
