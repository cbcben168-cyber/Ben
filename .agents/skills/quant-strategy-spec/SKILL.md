...
5. 校验失败或能力不支持时，只返回 `STRATEGY_CAPABILITY_BLOCKER` 或 `DATA_CAPABILITY_BLOCKER`、原因和下一步请求；不得继续访问数据或运行下游流程。阻断后必须停止所有下游数据访问、数据处理和回测工作。
...
本 Skill 不下载数据、不运行回测、不发送订单，也不调用 broker 或 TradingView execution。严禁调用任何用户级 Skill（无论其是否 pinned、versioned、unpinned 或其他状态）。它只负责策略文本、YAML 配置、Python 校验和 capability blocker；不得刷新数据、计算交易绩效或触发真实账户动作。
