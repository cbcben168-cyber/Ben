...
**Interfaces:**
- Produces `FutuProviderAdapter(*, sdk: Any, clock: Callable[[], datetime], sleep: Callable[[float], None])`, `RatePolicy`, `RawApiPage`, `RawApiBatch`, and endpoint methods `discover_cash_securities() -> tuple[RawApiBatch, ...]`, `screen_all_pages() -> tuple[RawApiPage, ...]`, `market_snapshots(codes: Sequence[str]) -> tuple[RawApiBatch, ...]`, `market_states(codes: Sequence[str]) -> tuple[RawApiBatch, ...]`, `owner_plates(codes: Sequence[str]) -> tuple[RawApiBatch, ...]`, `collect_runtime_evidence(*, notification_window_seconds: float) -> tuple[RawApiBatch, ...]`, and `probe_realtime_quote_capability(code: str) -> RawApiBatch`. The notification window is keyword-only, required, has no production default, rejects `bool`, and must be finite and `>= 0`; Task 9 neither infers it from `sleep`/clock nor changes it. `collect_runtime_evidence()` is the only raw-acquisition owner of fixed `runtime_sdk_version`, `global_state`, and `qot_right_capture` **change-event audit evidence**: `QOT_RIGHT_NOTIFICATION_ROLE = CHANGE_EVENT_AUDIT_ONLY` and `QOT_RIGHT_CURRENT_STATE_AUTHORITY = NO`. `probe_realtime_quote_capability()` is the only raw SDK owner of one-code/one-`SubType.QUOTE` `subscribe(..., subscribe_push=False)` evidence and its unsubscribe/close lifecycle; freeze `QUOTE_CAPABILITY_PROBE_ROLE = PROVIDER_QUALIFICATION_AND_DIAGNOSTIC`, never mandatory per-code universe production acquisition.
...
- [ ] **Step 1: RED** — Instantiate `FutuProviderAdapter(sdk=fake_sdk, clock=fake_clock, sleep=fake_sleep)` and test discovery categories, full pagination, raw request/response hashes, ret-code mapping, actual Market Snapshot row shape (including absence of synthetic freshness fields), raw per-security `get_market_state` batches, and `collect_runtime_evidence(notification_window_seconds=...)` for its three fixed raw evidence endpoints. Test the required finite non-bool non-negative window with no default and QOT_RIGHT 0/5/15-second no-event audit semantics; it may never infer current rights. Test `probe_realtime_quote_capability()` with one code × `SubType.QUOTE`, `subscribe_push=False`, no history/no scan/no long-lived push, raw subscribe/request/response/ret/error/timestamps/SDK/OpenD/hash, and separate `capability_verdict`/`cleanup_verdict`: subscribe success remains narrowly PROVEN even if cleanup is unresolved; a failed subscribe is UNKNOWN (never `NO_RIGHT`); after the documented one-minute minimum it must explicitly unsubscribe/query/close, while early close records `DELAYED_RELEASE_RISK`. Preserve schema nulls, bounded retry, independent 400/60 and 200/10 limiters, and context close on success/failure.
- [ ] **Step 2: Confirm RED** — Run `pytest tests/pattern_finder/universe_foundation/test_futu_adapter.py -q`; expected missing adapter.
- [ ] **Step 3: GREEN** — Implement raw endpoint/notification/probe acquisition only. The qualification/diagnostic probe uses a dedicated context and one code × one `SubType.QUOTE`; it records separate capability and cleanup verdicts, waits through the documented minimum before explicit unsubscribe/query/close or records early-close `DELAYED_RELEASE_RISK`, and never assumes close instantly releases quota. It makes no identity, Active mapping, current-right/delay mapping, classification, threshold, completeness or membership decision and is not called once per Universe security.
...
- [ ] **Step 6: Independent review** — Confirm endpoint contracts, pagination, quota-safe probe lifecycle, QOT_RIGHT audit-only role, raw hashes and no direct downstream SDK path; record the interface handoff PASS before Task 10.
...
- Produces `FutuUniverseGateway(*, sdk: Any | None=None, host: str="127.0.0.1", port: int=11111, clock: Callable[[], datetime], sleep: Callable[[float], None])`, `GatewayPreflight`, `GatewayAttempt`, `ApiBatchRecord`, `ActiveStatusMappingEntry`, `QualifiedActiveStatusMapping`, `QualifiedMarketStateConsistencyContract`, `IdentityLedgerEntry`, and the production entry:
  ...
  `runtime_evidence_window_seconds` is keyword-only, required, has no default, rejects `bool`, and must be finite and `>= 0`.
- `ActiveStatusMappingEntry` freezes `raw_value: str`, `decision: Decision`, and `reason_code: str`; entries accept only PASS/FAIL, require unique non-empty raw values/reasons, and sort deterministically. `QualifiedActiveStatusMapping` freezes `provider: str`, `provider_sdk_version: str`, `opend_server_version: str`, `mapping_version: str`, `entries: tuple[ActiveStatusMappingEntry, ...]`, `qualified_at_utc: datetime`, `qualification_references: tuple[EvidenceReference, ...]`, and its canonical SHA-256. Both versions are non-empty exact-match fields in the hash; no legacy SDK-only `provider_version` matching remains. It is provider-specific, dual-version-specific, versioned, auditable and qualification-backed; no wildcard/default PASS entry is allowed.
...
`GatewayAttempt` freezes `attempt_id: str`, `as_of_session: date`, `observed_at_utc: datetime`, `provider_update_time: datetime | None`, `runtime_evidence_window_seconds: float`, `market_data_delay_class: str`, `realtime_capability_probes: tuple[RawApiBatch, ...]`, `market_state_consistency_contract: QualifiedMarketStateConsistencyContract`, `active_status_mapping: QualifiedActiveStatusMapping`, `prerequisites_sha256: str`, `preflight: GatewayPreflight`, `evidence: tuple[UniverseSecurityEvidence, ...]`, `prerequisites: tuple[SecurityEvaluationPrerequisites, ...]`, `batches: tuple[ApiBatchRecord, ...]`, `identity_ledger: tuple[IdentityLedgerEntry, ...]`, `attempt_status: AttemptStatus`, `completeness: Completeness`, and `reason_codes: tuple[str, ...]`. The embedded mapping and state-consistency contract each preserve provider, exact SDK/OpenD versions, mapping version, qualification timestamp/references and canonical hash; detached hash-only bindings are forbidden. `realtime_capability_probes` is optional qualification/diagnostic audit evidence, may be an empty tuple, and its cardinality is never required to equal Universe securities cardinality; any present immutable result is code-bound and covered by `attempt_id` / canonical attempt hash. `runtime_evidence_window_seconds` and the state-contract hash remain attempt-acquisition inputs. `GatewayPreflight` may consume the resulting raw evidence but must not own, redefine, default, extend, shorten, or otherwise rewrite the window.
...
Task 10 also owns the attempt-level freshness gate. US snapshot `update_time` must be parsed as offset-less `America/New_York` local time and converted DST-aware to UTC; it must never be interpreted as naive UTC. FORMAL inputs require the latest complete XNYS regular session, exact binding of `as_of_session` / `observed_at_utc` / converted provider update time, actual consumption of each Task 9 raw `market_states(codes)` enum through the required immutable `QualifiedMarketStateConsistencyContract`, and a future qualified scalable provider/account/connection-level current quote-right authority. Task 10 is the sole state-contract owner/consumer; Task 9 supplies raw batches only. Missing, duplicate, unknown, unqualified, version/hash-mismatched or inconsistent states fail closed as `UNIVERSE_FRESHNESS_BLOCKER`; merely checking that every code has a row is forbidden. `global_state` is diagnostic-only. Task 10 must pass the unchanged window through exactly as `collect_runtime_evidence(notification_window_seconds=runtime_evidence_window_seconds)` and must not bypass Task 9 by directly calling `get_market_state`, `get_global_state`, `set_handler`, `subscribe` or `unsubscribe`. Market Snapshot does **not** supply `market_data_delay_class`, `regular_session_complete` or `market_session`: Task 10 must not read or synthesize them. QOT_RIGHT `us_qot_right` remains change-event audit evidence only: `events == []` means exactly `NO_QOT_RIGHT_EVENT_OBSERVED_DURING_CAPTURE_WINDOW`, regardless of 0/5/15 seconds, and neither empty nor present events prove LEVEL1/LEVEL2/BMP/NO_RIGHT/REALTIME/DELAYED. Optional raw probe `capability_verdict=PROVEN` proves only the matching connection/security/subtype/time; it cannot authorize other codes or the Universe, and `cleanup_verdict` remains independent. Freeze `PER_SECURITY_QUOTE_PROBE_FORMAL_REQUIREMENT = NO`, `PROVIDER_OR_ACCOUNT_LEVEL_CURRENT_QUOTE_RIGHT_AUTHORITY = NOT_YET_QUALIFIED`, and `FORMAL_FRESHNESS_AUTHORITY = NOT_QUALIFIED`. Therefore FORMAL still fails closed as `FAILED/INCOMPLETE/UNIVERSE_FRESHNESS_BLOCKER` with `CURRENT_QUOTE_RIGHT_AUTHORITY_NOT_QUALIFIED` semantics; Task 10 may implement this fail-closed engineering contract without claiming `PRODUCTION_FORMAL_READY`. `market_data_delay_class`, if retained, is audit `UNKNOWN` only. Intraday remains PREVIEW/PROVISIONAL. `EvidenceProvenance.observed_at_utc` remains audit data and is not a Task 6 freshness verdict.

Keep the authority levels explicit. Level 1 is the US.AAPL × QUOTE diagnostic probe and may record only `PROVEN_SCOPE_LIMITED`. Level 2 is a future auditable provider/account/connection-level current quote-right authority that does not require per-security subscriptions; it remains a research/qualification gap, and this plan approves no API or mapping for it. `global_state`, QOT_RIGHT notifications, and single-security probe success cannot be promoted to Level 2. For exact `10.10.7008 + 1009` qualification, freeze `ACTIVE_NORMAL_10_10_7008_1009 = QUALIFIED`, `MARKET_STATE_AFTER_HOURS_END_10_10_7008_1009 = QUALIFIED_OBSERVED_RELATIONSHIP`, and all other market states as `UNQUALIFIED_FAIL_CLOSED`.

- [ ] **Step 1: RED** — Every example calls `FutuUniverseGateway(sdk=fake_sdk, clock=fake_clock, sleep=fake_sleep)` and supplies immutable qualified Active and `QualifiedMarketStateConsistencyContract` objects plus a required `runtime_evidence_window_seconds`. Test the window and full state-contract object/hash are attempt-hash-sensitive; optional probe evidence may be absent, is not cardinality-bound to Universe securities, and any present result is code-bound/hash-sensitive. Require exact qualification timestamp/references/hash for `sdk=10.10.7008` / `OpenD=1009`: only `NORMAL` Active and the observed `AFTER_HOURS_END`/XNYS relationship qualify; historical `10.09.6908 + 1009` never falls back and other enums fail closed. Test every raw market-state value is consumed via the exact contract, including duplicate/missing/unknown/unqualified/mismatch rejection. Test QOT_RIGHT no-event and events-present audit semantics without a current-right inference; a probe success has only connection/security/subtype/time scope and cannot authorize the Universe. Test FORMAL remains blocked by unqualified scalable current quote-right authority even with a successful AAPL probe. Retain distinct capability/cleanup verdicts, no direct SDK state/handler/subscribe/unsubscribe, Active guard, identity, provenance, snapshot-shape and no-partial-publication tests.
...
- [ ] **Step 3: GREEN** — Compose Task 9 adapter outputs, including `market_states(codes)`, `collect_runtime_evidence(notification_window_seconds=runtime_evidence_window_seconds)`, optional qualification/diagnostic probe evidence, and the required full `QualifiedMarketStateConsistencyContract`, without rewriting the window; apply only the qualified exact SDK+OpenD-version Active mapping after explicit bool guards; execute the single freshness gate from snapshot time, XNYS calendar, actually consumed per-security market state and the future scalable current quote-right authority. Until that authority is qualified, return `UNIVERSE_FRESHNESS_BLOCKER` without partial FORMAL; do not acquire a probe per Universe code. Freeze required acquisition inputs and any optional probe evidence in the canonical attempt record/hash; reconcile identities, produce Task 6 contract values, compute `prerequisites_sha256`, embed complete mapping and state-contract objects, normalize evidence, and close atomically. Do not request history K-lines, evaluate profile thresholds, call Task 6 evaluator, implement OpenFIGI, alter Task 9 raw semantics, or treat QOT_RIGHT as current authority.
...
- [ ] **Step 6: Independent review and quota-safe qualification** — Record the approved `10.10.7008 + 1009` evidence: US.AAPL Active `NORMAL/false/false`, market state `AFTER_HOURS_END` consistent with XNYS non-session, QUOTE quota `300 → 299 → 300` across `>=60s` and explicit unsubscribe, plus about 60.97 seconds with no QOT_RIGHT event. Confirm Task 10 is the sole owner/consumer of the full hash-bound `QualifiedMarketStateConsistencyContract`, provider freshness and identity reconciliation; Task 9 remains the sole raw owner of `market_states()`, QOT_RIGHT audit capture and probe lifecycle; unqualified market-state enums fail closed without blocking Task 10 engineering; raw enums do not leak to Task 6; QOT_RIGHT never supplies current authority; capability and cleanup verdicts remain distinct; the probe stays qualification/diagnostic only and is never required per security; FORMAL remains blocked until scalable current quote-right authority is qualified; no synthetic snapshot freshness field, naive-UTC parse, hydration, semantic fallback, partial FORMAL result, duplicate freshness code or policy merge exists. OpenFIGI remains contract research only. Verify automated producer contract tests and record PASS before Task 11; no UI manual gate occurs here.
...
- [ ] Active Status raw mapping exists only in Task 10, is exact FUTU SDK/OpenD provider-version/mapping qualified, has live-sample + SDK enum-introspection + official-doc references, and unrecognized/null/incomplete mapping values become UNKNOWN/Quarantine rather than PASS.
- [ ] Evidence/provider freshness arithmetic and qualified market-state consistency interpretation exist only in Task 10 and use only `UNIVERSE_FRESHNESS_BLOCKER`; Task 9 only captures raw snapshot time, per-security market state, QOT_RIGHT change-event audit evidence and one-code `SubType.QUOTE` qualification/diagnostic probe evidence. Task 10 actually consumes enum values, never merely row cardinality; QOT_RIGHT is never current-right authority; no per-security probe gate exists; FORMAL stays closed until scalable current quote-right authority is qualified; downstream modules consume the verdict without recalculation.
