...
- [ ] **Step 1: RED** — Instantiate `FutuProviderAdapter(sdk=fake_sdk, clock=fake_clock, sleep=fake_sleep)` and test discovery categories, full pagination, raw request/response hashes, ret-code mapping, actual Market Snapshot row shape (including absence of synthetic freshness fields), raw per-security `get_market_state` batches, raw QOT_RIGHT `us_qot_right` notification capture/OpenD version, schema null preservation, bounded retry, 400/60 and 200/10 independent limiters, and context close on success/failure.
...
- [ ] **Step 3: GREEN** — Compose Task 9 adapter outputs, apply only the qualified exact SDK+OpenD-version Active mapping after explicit bool guards, execute the single attempt freshness gate from real snapshot time, XNYS calendar, per-security market state and qualified quote-right evidence, reconcile identities into composite-key rows, produce Task 6 contract values, compute `prerequisites_sha256`, embed the complete mapping object, normalize evidence, and close the attempt atomically. Do not request history K-lines, evaluate profile thresholds, call Task 6 evaluator, or implement OpenFIGI.
...
- [ ] Evidence/provider freshness arithmetic and delay/session interpretation exist only in Task 10 and use only `UNIVERSE_FRESHNESS_BLOCKER`; Task 9 only captures raw snapshot time, per-security market state and QOT_RIGHT evidence, and downstream modules consume the verdict without recalculation.
