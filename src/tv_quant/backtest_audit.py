...
def _check_costs(context: AuditContext) -> tuple[bool, list[AuditIssue], list[str]]:
    if context.trades.empty:
        return True, [], []
    required = ("gross_notional", "commission", "slippage_bps")
    if any(column not in context.trades for column in required):
        return False, [AuditIssue(
            "COST_MISMATCH", "ERROR", "trades are missing commission or slippage evidence"
        )], []
    try:
        for _, trade in context.trades.iterrows():
            expected_commission = float(trade["gross_notional"]) * context.spec.commission_bps / 10_000
            if not math.isclose(
...
def _check_artifacts(context: AuditContext) -> tuple[bool, list[AuditIssue], list[str]]:
    required_when_present = {"summary", "equity", "trades", "manifest", "audit"}
    missing = [
        name for name, path in context.artifact_paths.items()
        if name in required_when_present and (path is None or not Path(path).is_file())
    ]
    if missing:
        return False, [AuditIssue(
...
def _check_sample_and_concentration(
    ...
    if len(positive_growth) > 1 and total_growth > 0 and max(positive_growth.values()) / total_growth >= 0.80:
        issues.append(AuditIssue(
