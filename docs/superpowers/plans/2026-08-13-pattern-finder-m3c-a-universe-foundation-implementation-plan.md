...
**Interfaces:**
- Produces `FutuProviderAdapter(*, sdk: Any, clock: Callable[[], datetime], sleep: Callable[[float], None])`, `RatePolicy`, `RawApiPage`, `RawApiBatch`, and endpoint methods `discover_cash_securities() -> tuple[RawApiBatch, ...]`, `screen_all_pages() -> tuple[RawApiPage, ...]`, `market_snapshots(codes: Sequence[str]) -> tuple[RawApiBatch, ...]`, `market_states(codes: Sequence[str]) -> tuple[RawApiBatch, ...]`, `owner_plates(codes: Sequence[str]) -> tuple[RawApiBatch, ...]`, and `collect_runtime_evidence(*, notification_window_seconds: float) -> tuple[RawApiBatch, ...]`. The notification window is keyword-only, required, has no production default, rejects `bool`, and must be finite and `>= 0`; Task 9 neither infers it from `sleep`/clock nor changes it. `collect_runtime_evidence()` is the only raw-acquisition owner of the fixed `runtime_sdk_version`, `global_state`, and `qot_right_capture` endpoint evidence; it captures raw `QOT_RIGHT.us_qot_right` notification evidence and exact OpenD version without assigning a delay class.
...
- Produces `FutuUniverseGateway(*, sdk: Any | None=None, host: str="127.0.0.1", port: int=11111, clock: Callable[[], datetime], sleep: Callable[[float], None])`, `GatewayPreflight`, `GatewayAttempt`, `ApiBatchRecord`, `ActiveStatusMappingEntry`, `QualifiedActiveStatusMapping`, `IdentityLedgerEntry`, and the production entry:
...
`GatewayAttempt` freezes `attempt_id: str`, `as_of_session: date`, `observed_at_utc: datetime`, `provider_update_time: datetime | None`, `runtime_evidence_window_seconds: float`, `market_data_delay_class: str`, `active_status_mapping: QualifiedActiveStatusMapping`, `prerequisites_sha256: str`, `preflight: GatewayPreflight`, `evidence: tuple[UniverseSecurityEvidence, ...]`, `prerequisites: tuple[SecurityEvaluationPrerequisites, ...]`, `batches: tuple[ApiBatchRecord, ...]`, `identity_ledger: tuple[IdentityLedgerEntry, ...]`, `attempt_status: AttemptStatus`, `completeness: Completeness`, and `reason_codes: tuple[str, ...]`. The embedded mapping preserves provider, exact SDK version, exact OpenD version, mapping version, entries, qualification timestamp, qualification references and canonical hash; a detached hash-only binding is forbidden. `runtime_evidence_window_seconds` is an immutable attempt-acquisition parameter and must be covered by `attempt_id` / the canonical attempt hash: changing the capture window must change attempt identity/hash. `GatewayPreflight` may consume the resulting raw evidence but must not own, redefine, default, extend, shorten, or otherwise rewrite the window.
...
Task 10 also owns the attempt-level freshness gate. US snapshot `update_time` must be parsed as offset-less `America/New_York` local time and converted DST-aware to UTC; it must never be interpreted as naive UTC. FORMAL inputs require the latest complete XNYS regular session, exact binding of `as_of_session` / `observed_at_utc` / converted provider update time, and Task 9 raw `market_states(codes)` as the per-security session authority; `global_state` is diagnostic-only. XNYS calendar decides session and regular close. Task 10 must pass the unchanged value through exactly as `collect_runtime_evidence(notification_window_seconds=runtime_evidence_window_seconds)` and must not bypass Task 9 by directly calling `get_market_state`, `get_global_state`, or `set_handler`. Market Snapshot does **not** supply `market_data_delay_class`, `regular_session_complete` or `market_session`: Task 10 must not read or synthesize them. Delay evidence is raw Task 9 QOT_RIGHT `us_qot_right`; `qot_right_capture.events == []` means only `NO_QOT_RIGHT_EVENT_OBSERVED_DURING_CAPTURE_WINDOW`, whose audit meaning is bound to `runtime_evidence_window_seconds`, and does not prove `REALTIME`, `DELAYED`, or `NO_RIGHT`. Events present also remain `UNKNOWN` unless a later approved mapping exactly binds SDK version, OpenD version, raw `us_qot_right` value, official/provider qualification evidence, and mapping version. Until that documented exact mapping exists, `market_data_delay_class = UNKNOWN` and FORMAL remains `FAILED/INCOMPLETE/UNIVERSE_FRESHNESS_BLOCKER`. Intraday remains PREVIEW/PROVISIONAL. `EvidenceProvenance.observed_at_utc` remains audit data and is not a Task 6 freshness verdict.

- [ ] **Step 1: RED** — Every example calls `FutuUniverseGateway(sdk=fake_sdk, clock=fake_clock, sleep=fake_sleep)` and supplies an immutable qualified mapping plus a required `runtime_evidence_window_seconds`. Test the gateway window is keyword-only, finite, non-bool, non-negative and default-free; it reaches Task 9 unchanged as `notification_window_seconds`, freezes in `GatewayAttempt`, and changes `attempt_id` / canonical attempt hash when changed. Test exact FUTU SDK **and OpenD** provider-version/mapping-version matching, full live-sample + SDK enum introspection + official-doc qualification timestamp/references/hash persistence, and mapping tamper sensitivity; the `NORMAL` PASS candidate is permitted only for the exact `sdk=10.09.6908` / `OpenD=1009` qualification record, never as fallback. Test delisting-before-suspension deterministic FAIL priority; each flag individually null/missing/unparseable, both flags missing, and neither-explicitly-false → Active UNKNOWN; mapped PASS/FAIL only after both explicit false; new enum/null/missing mapping entry → Active UNKNOWN/Quarantine; one prerequisite per `(stock_id, futu_code)` evidence key plus deterministic `prerequisites_sha256`; same-stock-id/multiple-code preserves every row with per-key Identity UNKNOWN; same-code/multiple-stock-id whole-attempt failure before join; unfinished reconciliation no PASS; subtype evidence/provenance; actual snapshot row shape; `America/New_York` DST-aware update-time conversion; XNYS close plus Task 9 `market_states(codes)`; the three Task 9 runtime evidence endpoints; no direct SDK `get_market_state`/`get_global_state`/`set_handler` call; QOT_RIGHT no-event semantics bound to the attempt window; and every absent/unqualified QOT_RIGHT delay state failing only as `UNIVERSE_FRESHNESS_BLOCKER`. Failed necessary batch remains `FAILED/INCOMPLETE/non-formal`; no partial publication.
...
- [ ] **Step 3: GREEN** — Compose Task 9 adapter outputs, including `market_states(codes)` and `collect_runtime_evidence(notification_window_seconds=runtime_evidence_window_seconds)` without rewriting the required window; apply only the qualified exact SDK+OpenD-version Active mapping after explicit bool guards; execute the single attempt freshness gate from real snapshot time, XNYS calendar, per-security market state and qualified quote-right evidence; freeze the window in the canonical attempt record/hash; reconcile identities into composite-key rows; produce Task 6 contract values; compute `prerequisites_sha256`; embed the complete mapping object; normalize evidence; and close the attempt atomically. Do not request history K-lines, evaluate profile thresholds, call Task 6 evaluator, implement OpenFIGI, or alter Task 9 raw semantics.
...
- [ ] **Step 6: Independent review and quota-safe qualification** — Record actual approved tiny-sample SDK/OpenD fields, exact provider/provider-version/mapping versions and qualification evidence or stable blocker. Confirm Task 10 is the sole Active mapping, provider freshness and identity reconciliation owner; Task 9 remains the sole raw owner of `market_states()` and runtime evidence capture; the required attempt window is unchanged at the Task 9 handoff, immutable in `GatewayAttempt`, and attempt-hash-sensitive; `GatewayPreflight` is not a second window owner; raw enums do not leak to Task 6; no synthetic snapshot freshness field, naive-UTC time parse, unqualified QotRight-to-REALTIME mapping, or no-event inference exists; UNKNOWN fails closed; both identity conflict modes remain distinct; no hydration, semantic fallback, partial FORMAL result, duplicate freshness code or policy merge exists. OpenFIGI remains contract research only until the Common Stock/ADR/Preferred/Unit real-response qualification matrix passes. Verify automated producer contract tests and record PASS before Task 11; no UI manual gate occurs here.
...
- [ ] Evidence/provider freshness arithmetic and delay/session interpretation exist only in Task 10 and use only `UNIVERSE_FRESHNESS_BLOCKER`; Task 9 only captures raw snapshot time, per-security market state, and fixed runtime evidence (`runtime_sdk_version`, `global_state`, `qot_right_capture`) under its caller-supplied `notification_window_seconds`, and downstream modules consume the verdict without recalculation.
