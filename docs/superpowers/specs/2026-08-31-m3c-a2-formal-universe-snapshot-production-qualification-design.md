...
- canonical `active_status_mapping_sha256`, `prerequisites_sha256`, member, content, and record hashes, each with its existing canonical definition.
...
Canonical hash dependency DAG
  QualifiedActiveStatusMapping canonical content
    -> active_status_mapping_sha256
  canonical SecurityEvaluationPrerequisites collection
  ...
  EvidenceArtifact hashes + ApiAcquisitionBracket hashes
    ...
    -> qualification_provenance_sha256
  active_status_mapping_sha256 + prerequisites_sha256 + qualification_provenance_sha256
    + other frozen GatewayAttempt canonical fields
    -> GatewayAttempt canonical payload
    -> gateway_attempt_sha256
  qualified evaluations/funnel/rows and their existing domain outputs
    + frozen profile, mapping, prerequisite, provenance, and attempt bindings
    -> snapshot_content_sha256
    -> snapshot_record_sha256
    -> PersistedFinalAuditProjection
```
...
- `gateway_attempt_sha256`;
- any redefined `active_status_mapping_sha256` or `prerequisites_sha256` derived from provenance, attempt, or snapshot state;
...
No excluded value may be read, defaulted, or projected back into the provenance canonical serializer. `active_status_mapping_sha256` remains the canonical-content hash of `QualifiedActiveStatusMapping`, and `prerequisites_sha256` remains the canonical-content hash of the ordered `SecurityEvaluationPrerequisites` collection. Both exist before `GatewayAttempt` is frozen, are independent already-frozen inputs alongside `qualification_provenance_sha256`, and are neither redefined by v2 nor derived from provenance. `GatewayAttempt` canonical payload may and must bind all three. Neither existing upstream hash may include `qualification_provenance_sha256`, `gateway_attempt_sha256`, or any snapshot hash. Snapshot v2 header fields preserve and exact-revalidate those same existing upstream hashes; only `snapshot_content_sha256` and `snapshot_record_sha256` aggregate-bind the provenance, attempt, mapping, prerequisite, profile, factor/version, funnel/member, and row/decision bindings. The direction is therefore frozen upstream inputs to attempt to snapshot only. Changing a downstream display, out-of-band audit, persistence metadata, or final hash value cannot change valid recomputation of an upstream mapping, prerequisite, or provenance hash.
...
`active_status_mapping` and `prerequisites_sha256` are existing `GatewayAttempt` fields, not v2 additions or aliases. Before a future v2 attempt is frozen, `active_status_mapping_sha256` is recomputed only from the `QualifiedActiveStatusMapping` canonical content and `prerequisites_sha256` only from the canonical ordered `SecurityEvaluationPrerequisites` collection. The future attempt canonical payload consumes those exact upstream hashes together with `qualification_provenance_sha256`; it never recomputes either upstream identity after its own hash exists.
...
15. Only after Step 14, run existing per-security evaluation and `build_funnel()`. The evaluator consumes qualified business evidence and thresholds only; it never decides provider compatibility.
16. Preserve and exact-revalidate the already-frozen `active_status_mapping_sha256` and `prerequisites_sha256` from `GatewayAttempt`; never recompute them from the attempt hash, provenance hash, or any snapshot hash. Compute existing member/funnel and other domain outputs only from their own canonical business inputs.
17. Call `build_snapshot(kind=FORMAL, ...)` with deterministic identity, the exact service-resolved qualification set/provider envelope/factor contracts, future bracket/clock evidence, evidence-manifest hash and immutable artifact references, locked-session evidence, verdicts, `qualification_provenance_sha256`, `gateway_attempt_sha256`, and the unchanged upstream mapping/prerequisite hashes. The builder emits only `universe-snapshot/v2` for this path. Then compute `snapshot_content_sha256` and, from that frozen aggregate, `snapshot_record_sha256`; those two downstream hashes jointly bind the required provenance, attempt, mapping, prerequisite, profile, factor/version, member/funnel, and row/decision values. Assemble `PersistedFinalAuditProjection` only after the record hash exists.
18. Call only `SnapshotRepository.append(snapshot, evidence_manifest, evidence_artifacts)`. Its one SQLite transaction stores the accepted snapshot aggregate, manifest, referenced canonical artifacts, and any downstream final-audit projection. SQLite does not decide qualification or membership.
19. After process-independent reopen with `SnapshotRepository.get(snapshot_id)`, recompute and exact-compare in this topological order: mapping canonical content and `active_status_mapping_sha256`; canonical prerequisite collection and `prerequisites_sha256`; artifact/acquisition-bracket hashes; manifest-entry/root hashes; individual qualification/clock/temporal/capability/verdict hashes; provenance projection/hash; the `GatewayAttempt` canonical payload and `gateway_attempt_sha256`; evaluation/funnel/domain outputs; and finally the Snapshot v2 `snapshot_content_sha256` and `snapshot_record_sha256`. Return `SUCCEEDED` only after every layer verifies. Replay must never use a stored attempt or snapshot hash to reconstruct the mapping or prerequisite content, and must never use stored `snapshot_record_sha256` or any other downstream hash to reconstruct provenance; a missing layer, hash mismatch, serializer mismatch, or canonical-order mismatch fails closed.
...
Before this method writes, the producer must complete the Section 5.6/7.3 topology in memory: recompute/freeze `QualifiedActiveStatusMapping` canonical content and `active_status_mapping_sha256`; recompute/freeze the canonical prerequisite collection and `prerequisites_sha256`; freeze canonical provider evidence; compute artifact/acquisition-bracket hashes; compute manifest entry/root hashes; compute individual qualification/clock/temporal/capability/verdict hashes; construct/hash `FormalQualificationProvenance`; construct/freeze/hash `GatewayAttempt` using those already-frozen upstream mapping, prerequisite, and provenance hashes; run evaluator/funnel and existing domain outputs; build Snapshot v2; then compute `snapshot_content_sha256` and `snapshot_record_sha256`. Every input at each layer must already exist and be immutable. No persistence-generated row hash or downstream audit projection may feed any earlier layer.
...
`SnapshotRepository.get(snapshot_id)` first reopens the immutable snapshot aggregate, then loads the manifest by the header's exact `evidence_manifest_sha256`, resolves every artifact reference, and selects the decoder only by the stored `serializer_id + serializer_version`. It then replays the same dependency DAG: recompute and exact-compare `QualifiedActiveStatusMapping` canonical content and `active_status_mapping_sha256`; the canonical prerequisite collection and `prerequisites_sha256`; artifact/acquisition-bracket hashes; manifest entry order/hashes and root hash; individual provider/factor/clock/temporal/capability/verdict objects and hashes; the exact `FormalQualificationProvenance` canonical projection and `qualification_provenance_sha256`; the frozen `GatewayAttempt` canonical payload and `gateway_attempt_sha256`; evaluation/funnel/domain outputs; and finally Snapshot v2 `snapshot_content_sha256` and `snapshot_record_sha256`. Stored attempt/content/record hashes are comparison targets only and must never be supplied as provenance inputs or used to reconstruct mapping/prerequisite content.
...
42. An explicit dependency/topological-order test constructs the hash DAG in the Section 5.6/7.3 order: `QualifiedActiveStatusMapping` canonical content -> `active_status_mapping_sha256`; canonical `SecurityEvaluationPrerequisites` collection -> `prerequisites_sha256`; frozen evidence -> artifact/acquisition-bracket hashes -> manifest entry/root hashes -> individual qualification/clock/temporal/capability/verdict hashes -> `FormalQualificationProvenance` -> `qualification_provenance_sha256`; then those already-frozen upstream hashes plus the other `GatewayAttempt` canonical fields -> `GatewayAttempt` canonical payload -> `gateway_attempt_sha256`; then qualified evaluations/funnel/rows and the frozen bindings -> `snapshot_content_sha256` -> `snapshot_record_sha256` -> `PersistedFinalAuditProjection`. It proves the existing v1/v2 header equality assertions for mapping and prerequisite hashes, no self-reference, and no mapping/prerequisite-to-attempt cycle. A mapping-content mutation changes its mapping hash, then attempt and snapshot content/record hashes; a prerequisite-content mutation changes its prerequisite hash, then attempt and snapshot content/record hashes. A provenance-only mutation changes `qualification_provenance_sha256`, then attempt and snapshot content/record hashes, but leaves both mapping and prerequisite hashes unchanged. A downstream snapshot-only or audit mutation cannot change mapping/prerequisite/provenance hashes or attempt upstream objects. Attempt/snapshot downstream hashes are absent from the provenance serializer; restart recomputation uses the identical DAG and canonical orders. Missing layers, hash mismatches, serializer mismatches, and canonical-order mismatches all fail closed.
...
- exact `FormalQualificationProvenance` schema/projection and `qualification_provenance_sha256`, its explicit exclusion of attempt/snapshot/persistence-row/final-audit hashes, the separately recomputed upstream `active_status_mapping_sha256` and `prerequisites_sha256`, the topological build/restart trace, `gateway_attempt_sha256`, and the downstream-only `PersistedFinalAuditProjection`;
- gateway status/completeness/formal-ready/reason codes;
- S0-S10 totals, member/fail/quarantine counts, and reconciliation;
- persisted snapshot ID plus the existing upstream mapping/prerequisite hashes, member/funnel domain hashes, and snapshot content/record hashes;
