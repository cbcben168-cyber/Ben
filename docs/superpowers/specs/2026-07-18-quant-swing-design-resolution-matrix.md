...
6. 提前收市日由XNYS日历识别，不根据数据数量猜测。`expected_bar_count=(session_close-session_open)/30分钟`，例如09:30至13:00必须有7根30分钟柱；实际柱数量、起止时间和间隔必须与XNYS完全一致。缺失、重复、重叠、乱序或错误长度时状态为`DATA_QUALITY_FAILED`，该日数据不得用于日线OHLC、持仓估值、止损判断或下一交易日D-1指标。数据完整时允许生成日线供下一交易日使用，但当天不生成研究信号，只允许已有仓位风险处理和估值。DST先本地化再转UTC，禁止使用固定UTC偏移。
...
3. 每根30分钟柱唯一事件顺序为：开盘处理已有仓位跳空止损；处理上一根完成柱生成的清仓订单；处理减仓订单；处理入场/加仓订单；退出与入场冲突时退出优先且入场取消并记录冲突；随后用本柱low检查止损；新仓可在同柱触发止损；每个时间戳最多一个最终目标仓位订单；收盘更新估值，完整60分钟柱收盘后更新指标和状态并生成下一根可交易柱订单。普通买入成交价为`raw_open*(1+normal_slippage_bps/10000)`；普通卖出、减仓或清仓成交价为`raw_open*(1-normal_slippage_bps/10000)`；数据末日强制平仓成交价为`raw_close*(1-normal_slippage_bps/10000)`；禁止价格改善。每个fill独立按成交名义金额计算手续费，并保存`reference_price`、`fill_price`、`slippage_bps`、`commission`、`side`、`order_id`和`fill_id`。15:30–16:00辅助柱不生成新策略信号。
...
2. 交易次数为已完成生命周期数；`win_rate = 净PnL大于0的已完成trade_lifecycle_id数量 / 全部已完成trade_lifecycle_id数量`。没有已完成交易时`win_rate=null`且`win_rate_status=NO_COMPLETED_TRADES`。Profit Factor无亏损交易时为`null`且状态为`NO_GROSS_LOSS`，无完成PnL时为`null`且状态为`NO_COMPLETED_PNL`；Sharpe在日收益标准差为0时为`null`且状态为`ZERO_VARIANCE`；最大回撤基于每日收盘账户权益；Buy and Hold最大回撤为0时相对回撤改善为`null`、关卡状态为`NOT_APPLICABLE`且候选状态为`HOLD_BENCHMARK_DRAWDOWN_NOT_APPLICABLE`；该情况既不算通过，也不直接算策略失败；权益归零或为负时CAGR为`null`且状态为`CAPITAL_DEPLETED`并直接淘汰。报告禁止用0替代无定义指标。同时报告交易日数和完整60分钟柱数；资金利用率为每根30分钟柱`abs(position_market_value)/equity`的时间加权平均；MFE/MAE使用生命周期内30分钟raw high/low、初始加权平均入场价和冻结的初始1R。
...
9. 报告机器字段至少包括`schema_version`、`dataset_id`、`data_manifest_hash`、`candidate_id`、`experiment_id`、`strategy_family`、`config_hash`、`code_commit`、`stage`、`cost_scenario`、`benchmark_type`、`run_id`、`result_id`、`data_start`、`data_end`、`total_return`、`CAGR`、`max_drawdown`、`Sharpe`、`Profit_Factor`、`trade_count`、`win_rate`、`win_rate_status`、`relative_drawdown_improvement`、`relative_drawdown_gate`、`candidate_status`、`average_holding_days`、`average_holding_bars`、`capital_utilization`、`MFE`、`MAE`、`role_decisions`、`veto_status`、`veto_reason`、`contamination_status`、`approval_status`、`metric_status`和`generated_at_utc`。机器结果唯一键统一为`dataset_id + candidate_id + experiment_id + stage + cost_scenario + benchmark_type`；每次程序运行另生成不可变`run_id`和`result_id`。JSON、CSV和DuckDB使用同一业务唯一键，不得定义冲突主键；以`fill_id`、`order_id`、`trade_lifecycle_id`和`oos_attempt_id`关联明细和审计记录。
...
|---|---|---|---|---|---|---|---|---|
| B-01 | BLOCKER | 原设计4.2、4.3、30 | 采用XNYS日历、America/New_York本地化、UTC开始时间；正常日固定13根RTH 30分钟柱，提前收市日按`expected_bar_count=(session_close-session_open)/30分钟`计算预期柱数；完整性失败时阻断所有下游使用。 | 正常日必须有09:30至15:30共13根RTH柱；提前收市日实际柱数量、起止时间和间隔必须与XNYS一致，例如09:30至13:00为7根；缺失、重复、重叠、乱序或错误长度状态为`DATA_QUALITY_FAILED`，不得用于日线OHLC、持仓估值、止损判断或D-1指标。 | DST跨越、普通日、节假日、半日市、盘前盘后、预期柱数、起止时间、间隔、缺失/重复/重叠/乱序/错误长度fixture，以及失败数据下游阻断测试。 | 1 数据契约 | IMPLEMENTATION_PENDING | Futu `time_key`语义必须由fixture实测；供应商异常仍会导致数据集停用。 |
...
| H-07 | HIGH | 原设计2.1、16.3、18 | 20%为目标，>25%为绝对淘汰，回撤相对PRIMARY_PRICE_ONLY改善至少25%为适用时的硬门槛；硬门槛不可互相补偿。 | B&H最大回撤为0时，`relative_drawdown_improvement=null`、`relative_drawdown_gate=NOT_APPLICABLE`、`candidate_status=HOLD_BENCHMARK_DRAWDOWN_NOT_APPLICABLE`，不算通过，也不直接算策略失败；其他候选必须同时满足策略最大回撤≤25%及相对改善≥25%。 | 边界20%、25%、恰好25%、B&H零回撤状态、不可用关卡和适用时相对改善边界测试。 | 6 单ETF回测与报告 | IMPLEMENTATION_PENDING | 不适用状态必须进入委员会报告，不能被排名流程误当作通过。 |
...
| M-05 | MEDIUM | 原设计21、31 | 用同一六字段组合作为JSON、CSV、DuckDB机器结果业务唯一键；run_id和result_id只追加不可变运行/结果身份，不替代业务唯一键。 | `dataset_id + candidate_id + experiment_id + stage + cost_scenario + benchmark_type`是跨格式业务唯一键。 | JSON/CSV/DuckDB schema互相校验、重复业务键拒绝、同一run多结果关联和result_id不可重写测试。 | 6 单ETF回测与报告 | IMPLEMENTATION_PENDING | 迁移旧报告时必须显式标记旧schema，不能静默映射。 |
| M-06 | MEDIUM | 原设计2、16、18、21 | 冻结所有无定义指标的null和状态码，禁止以0代替；异常状态参与关卡判定。 | 无亏损为`NO_GROSS_LOSS`；无完成PnL为`NO_COMPLETED_PNL`；无完成交易胜率为`NO_COMPLETED_TRADES`；零方差为`ZERO_VARIANCE`；B&H零回撤为`NOT_APPLICABLE`并转为`HOLD_BENCHMARK_DRAWDOWN_NOT_APPLICABLE`；权益归零/为负为`CAPITAL_DEPLETED`并淘汰。 | 各状态边界、JSON null、CSV空值、胜率分子分母、B&H零回撤候选状态、权益归零和负权益测试。 | 6 单ETF回测与报告 | IMPLEMENTATION_PENDING | 小样本下null指标仍可能被误读，摘要必须同时显示状态码。 |
| H-15 | HIGH | 原设计4.4、10、11、15 | 冻结普通订单、止损订单和末日强平的成交价公式，禁止价格改善；每个fill独立计算手续费并保存完整成交字段。 | 普通买入为`raw_open*(1+normal_slippage_bps/10000)`；普通卖出、减仓、清仓为`raw_open*(1-normal_slippage_bps/10000)`；数据末日强平为`raw_close*(1-normal_slippage_bps/10000)`。 | 普通入场、加仓、减仓、清仓、尾盘新仓和末日强平分别测试reference_price、fill_price、side、滑点、手续费、order_id、fill_id及成交名义金额；同时测试禁止价格改善和止损公式不被覆盖。 | 4 事件引擎、5 仓位状态机 | IMPLEMENTATION_PENDING | 30分钟OHLC不能表达真实盘口，普通滑点仍是冻结的规则化模拟。 |
...
以下规则是本矩阵关于胜率、业务唯一键、Buy and Hold零回撤、成交价和提前收市完整性的最终权威定义；同一主题的早先概括以本节为准。

1. `win_rate`只使用已完成`trade_lifecycle_id`：净PnL大于0的完成生命周期数除以全部完成生命周期数；没有完成交易时为`null`并写入`win_rate_status=NO_COMPLETED_TRADES`。
2. 机器结果业务唯一键为`dataset_id + candidate_id + experiment_id + stage + cost_scenario + benchmark_type`。JSON、CSV和DuckDB必须使用相同业务唯一键；`run_id`和`result_id`分别是不可变运行身份和结果身份。
3. Buy and Hold最大回撤为0时，`relative_drawdown_improvement=null`、`relative_drawdown_gate=NOT_APPLICABLE`、`candidate_status=HOLD_BENCHMARK_DRAWDOWN_NOT_APPLICABLE`；该状态既不是通过，也不是策略失败。
4. 普通买入为`raw_open*(1+normal_slippage_bps/10000)`；普通卖出、减仓或清仓为`raw_open*(1-normal_slippage_bps/10000)`；数据末日强制平仓为`raw_close*(1-normal_slippage_bps/10000)`。禁止价格改善，手续费按每个fill的成交名义金额独立计算。每笔成交保存`reference_price`、`fill_price`、`slippage_bps`、`commission`、`side`、`order_id`和`fill_id`。
5. 提前收市日`expected_bar_count=(session_close-session_open)/30分钟`；实际柱数量、起止时间和间隔必须与XNYS一致。任一完整性失败状态为`DATA_QUALITY_FAILED`，失败数据不得用于日线OHLC、持仓估值、止损判断或下一交易日D-1指标。
...
4. Buy and Hold主基准不含分红和利息，不能代表完整Total Return；当其最大回撤为0时只能进入`HOLD_BENCHMARK_DRAWDOWN_NOT_APPLICABLE`，不能被排名流程解释为通过或失败。
...
10. 六字段业务唯一键不能防止同一结果被错误覆盖，因此`run_id`、`result_id`和不可变写入策略仍需由实现保证。
...
- 全部finding_id为：`B-01`、`B-02`、`B-03`、`B-04`、`B-05`、`B-06`、`H-01`、`H-02`、`H-03`、`H-04`、`H-05`、`H-06`、`H-07`、`H-08`、`H-09`、`H-10`、`H-11`、`H-12`、`H-13`、`H-14`、`H-15`、`L-01`、`L-02`、`M-01`、`M-02`、`M-03`、`M-04`、`M-05`、`M-06`，共29项。
- finding_id全局唯一；本文件中不存在重复finding_id。
