...
Task 10 also owns the attempt-level freshness gate. US snapshot `update_time` must be parsed as offset-less `America/New_York` local time and converted DST-aware to UTC; it must never be interpreted as naive UTC. FORMAL inputs require the latest complete XNYS regular session, exact binding of `as_of_session` / `observed_at_utc` / converted provider update time, and actual consumption of each Task 9 raw `market_states(codes)` enum through the required immutable `QualifiedMarketStateConsistencyContract`. Task 10 is its sole owner/consumer; Task 9 supplies raw batches only. Missing, duplicate, unknown, unqualified, version/hash-mismatched or inconsistent states fail closed as `UNIVERSE_FRESHNESS_BLOCKER`; merely checking that every code has a row is forbidden. `global_state` is diagnostic-only. Task 10 must pass the unchanged window through exactly as `collect_runtime_evidence(notification_window_seconds=runtime_evidence_window_seconds)`, consume per-code immutable capability/cleanup probe results, and must not bypass Task 9 by directly calling `get_market_state`, `get_global_state`, `set_handler`, `subscribe` or `unsubscribe`. Market Snapshot does **not** supply `market_data_delay_class`, `regular_session_complete` or `market_session`: Task 10 must not read or synthesize them. QOT_RIGHT `us_qot_right` remains change-event audit evidence only: `events == []` means exactly `NO_QOT_RIGHT_EVENT_OBSERVED_DURING_CAPTURE_WINDOW`, regardless of 0/5/15 seconds, and neither empty nor present events prove LEVEL1/LEVEL2/BMP/NO_RIGHT/REALTIME/DELAYED. Raw probe `capability_verdict=PROVEN` proves only the matching connection/security/subtype/time; it cannot authorize other codes. `cleanup_verdict` is independent: unresolved cleanup does not rewrite narrow capability success, but independently fails safety/FORMAL with `UNIVERSE_FRESHNESS_BLOCKER`. Any necessary code without PROVEN capability is UNKNOWN, never `NO_RIGHT`, and FORMAL fails closed. `market_data_delay_class`, if retained, is audit `UNKNOWN` only. Intraday remains PREVIEW/PROVISIONAL. `EvidenceProvenance.observed_at_utc` remains audit data and is not a Task 6 freshness verdict.

- [ ] **Step 1: RED** — Every example calls `FutuUniverseGateway(sdk=fake_sdk, clock=fake_clock, sleep=fake_sleep)` and supplies immutable qualified Active and `QualifiedMarketStateConsistencyContract` objects plus a required `runtime_evidence_window_seconds`. Test the window, full state-contract object/hash and per-code probe results are attempt-hash-sensitive; version/hash mismatch fails closed. Require fresh live-sample + SDK enum introspection + official-doc qualification timestamp/references/hash for `sdk=10.10.7008` / `OpenD=1009`; the historical `10.09.6908 + 1009` NORMAL candidate never falls back. Test every raw market-state value is consumed via the exact contract against XNYS, including `AFTERNOON`, `CLOSED`, pre/post/overnight values, duplicate/missing/unknown/unqualified/mismatch rejection. Test QOT_RIGHT 0/5/15-second no-event and events-present audit semantics without a current-right inference; a probe success has only connection/security/subtype/time scope, no unprobed code can be FORMAL, and `capability_verdict` remains distinct from cleanup failure/unknown. Retain no direct SDK state/handler/subscribe/unsubscribe, Active guard, identity, provenance, snapshot-shape and no-partial-publication tests.
...
- [ ] **Step 3: GREEN** — Compose Task 9 adapter outputs, including `market_states(codes)`, `collect_runtime_evidence(notification_window_seconds=runtime_evidence_window_seconds)`, per-code immutable raw probe results, and the required full `QualifiedMarketStateConsistencyContract`, without rewriting the window; apply only the qualified exact SDK+OpenD-version Active mapping after explicit bool guards; execute the single freshness gate from snapshot time, XNYS calendar, actually consumed per-security market state and only its matching capability/cleanup result. Freeze all acquisition inputs in the canonical attempt record/hash; reconcile identities, produce Task 6 contract values, compute `prerequisites_sha256`, embed complete mapping and state-contract objects, normalize evidence, and close atomically. Do not request history K-lines, evaluate profile thresholds, call Task 6 evaluator, implement OpenFIGI, alter Task 9 raw semantics, or treat QOT_RIGHT as current authority.
...
- [ ] **Step 6: Independent review and quota-safe qualification** — Record actual approved `10.10.7008 + 1009` tiny-sample fields and exact qualification evidence or stable blocker. Confirm Task 10 is the sole owner/consumer of the full hash-bound `QualifiedMarketStateConsistencyContract`, provider freshness and identity reconciliation; Task 9 remains the sole raw owner of `market_states()`, QOT_RIGHT audit capture and probe lifecycle; every market-state enum is consumed by the contract; raw enums do not leak to Task 6; QOT_RIGHT never supplies current authority; capability and cleanup verdicts remain distinct; neither a probe nor its result authorizes an unprobed code; UNKNOWN fails closed; no synthetic snapshot freshness field, naive-UTC parse, hydration, semantic fallback, partial FORMAL result, duplicate freshness code or policy merge exists. OpenFIGI remains contract research only. Verify automated producer contract tests and record PASS before Task 11; no UI manual gate occurs here.
